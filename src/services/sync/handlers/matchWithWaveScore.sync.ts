...
export function createMatchSyncWithWaveScore(
  ...
  // Helper function to calculate and update wave score for a match
  const calculateAndUpdateWaveScore = async (match: Match): Promise<Match> => {
    // Skip wave calculation if disabled or match already started
    if (!options?.calculateWaveScores) return match
    ...
    if (matchDate > maxDate) return match

    try {
      // Get team data from participants
      const participants = match.participants as any[]
      if (!participants || participants.length < 2) return match
      ...
      if (!homeParticipant || !awayParticipant) return match
      ...
      console.error(`Error calculating wave score for match ${match.id}:`, error)
      return match
    }
  }
  ...
  return createSyncService<SportmonksMatch>({
    ...
    afterCreate: calculateAndUpdateWaveScore,
    afterUpdate: async (updated: Match, existingDoc: Match) => {
      ...
      return calculateAndUpdateWaveScore(updated)
    }
  })
}